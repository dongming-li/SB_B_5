...
public class ShareSongWithUserController {
	...
	public @ResponseBody RequestResult addNewWorker(@RequestBody ShareSongWithUserDAO shareInfo) throws SQLException {
		...
		if (SongQueries.doesSongExist(shareInfo.getSongName(), connection)) {
			...
			if (Queries.doesUserExist(shareInfo.getSharedBy(), connection)) {
				
				if (Queries.doesUserExist(shareInfo.getSharedWith(), connection)) {
					...
					if (!wasSongAlreadyShared(shareInfo, connection)) {
						...
						result.setErrorMessage("None! The song " + shareInfo.getSongName() + " was shared by " 
								+ shareInfo.getSharedBy() + " with " + shareInfo.getSharedWith());
						result.setWasSuccess(true);
					} else {
						result.setErrorMessage("The song " + shareInfo.getSongName() + " was already shared with " + shareInfo.getSharedWith() + " by " + shareInfo.getSharedBy());
